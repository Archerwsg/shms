    ...
    <table cellpadding="0" cellspacing="0" border="0" class="table table-bordered">
    	<thead>
        	<tr>
                ...
                <th><?php echo get_phrase('select_cell');?></th>
		...
		<tbody>
        	<form method="post" action="<?php echo site_url('admin/attendance_selector');?>" class="form">
            	<tr class="gradeA">
                    ...
                    <td>
                    	<select name="cell_id" class="form-control selectboxit">
                        	<option value="">Select a cell</option>
                        	<?php 
							$cells	=	$this->db->get('cell')->result_array();
							foreach($cells as $row):?>
                        	<option value="<?php echo $row['cell_id'];?>"
                            	<?php if(isset($cell_id) && $cell_id==$row['cell_id'])echo 'selected="selected"';?>>
									<?php echo $row['name'];?>
                              			</option>
                            <?php endforeach;?>
                        </select>
                    ...
                    <td>
                        <select name="section_id" class="form-control selectboxit">
                            ...
                            <optgroup label="<?php echo $row['name'];?>">
                                <?php
                                    $sections = $this->db->get_where('section' , array('cell_id' => $row['cell_id']))->result_array();
...
<?php if($date!='' && $month!='' && $year!='' && $cell_id!=''):?>
    ...
    <div class="row">
        <div class="col-sm-offset-4 col-sm-4">
            ...
            <div class="tile-stats tile-white-gray">
                ...
                <h3>Attendance <?php echo ($cell_id);?></h3>
            ...
            <tbody>
                ...
                <?php 
                    $members   =   $this->db->get_where('enroll' , array(
                            'cell_id'=>$cell_id , 'year' => $running_year , 'section_id' => $section_id))->result_array();
...
<form method="post" 
    action="<?php echo site_url('admin/manage_attendance/'.$date.'/'.$month.'/'.$year.'/'.$cell_id.'/'.$section_id.'/'.$session);?>">
    <div class="col-sm-offset-3 col-md-6">
        <table  class="table table-bordered">
    		<thead>
    			<tr class="gradeA">
            ...
            <tbody>
    			...
    			foreach($students as $row)
    			{
    				...
    				<tr class="gradeA">
    					<td><?php echo $row['roll'];?></td>
    					<td>
                            <?php echo $this->db->get_where('member' , array('member_id' => $row['student_id']))->row()->name;?>               
                        </td>
    					<td align="center">
    						<?php 
    						//inserting blank data for students attendance if unavailable
    						$verify_data	=	array(	'member_id' => $row['member_id'],
    													'date' => $full_date,
                                                        'cell_id' => $cell_id , 
                                                        'section_id' => $section_id,
                                                        'year' => $session);
