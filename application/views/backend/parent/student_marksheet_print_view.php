...
<div id="print">
    ...
    <tbody>
        <?php
            ...
            $all = array_map(function($value){ return $value['subject_id'];} ,$all_subjects_for_students);
            $student->subjects = $student->subjects == null ? $all : json_decode($student->subjects , true);
            
            $subjects = $this->db->query("SELECT * FROM `subject` WHERE `class_id` = ".$class_id." AND subject_id IN (".implode(',', array_map('intval', $student->subjects)).")")->result_array();
        ...
        ?>
            <tr>
                ...
                <td style="text-align: center;"><?php if(isset($row4['mark_obtained'])){ echo $row4['comment'];}?></td>
            </tr>
        <?php endforeach;?>
    </tbody>
   </table>
