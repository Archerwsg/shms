...
class Student extends MY_Controller
{
    ...
    function subject($param1 = '', $param2 = '')
    {
        ...
        $student_subjects        = json_decode($this->db->get_where('enroll' , array(
            'student_id' => $student_profile->student_id,
                'year' => $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description
        ))->row()->subjects , true);

        $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $page_data['subjects']   = $this->db->query('SELECT * FROM `subject` WHERE (subject_id = "NULL") OR (subject_id IN ('.implode(',', array_map('intval', $student_subjects)).')) AND `year` = "'.$running_year.'"')->result_array();
