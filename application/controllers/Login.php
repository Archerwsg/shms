...
class Login extends MY_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('carer_login') == 1)
            redirect(site_url('carers/dashboard'), 'refresh');
    ...
    function validate_login() {
      ...
      $query = $this->db->get_where('carer', $credential);
      if ($query->num_rows() > 0) {
          $row = $query->row();
          $this->session->set_userdata('carer_login', '1');
          $this->session->set_userdata('carer_id', $row->carer_id);
          $this->session->set_userdata('login_user_id', $row->carer_id);
          $this->session->set_userdata('name', $row->name);
          $this->session->set_userdata('login_type', 'carer');
          redirect(site_url('carers/dashboard'), 'refresh');
      }
