...
class Sms_model extends CI_Model {
    ...
    function send_sms($message = '' , $reciever_phone = '')
    {
        ...
        if ($active_sms_service == 'zentech') {
            $this->send_sms_via_zentech($message , $reciever_phone );
        }
    ...
    function send_sms_via_zentech($message = '' , $reciever_phone = '') {
        $this->load->library('sender');
        
        $this->sender->strMessage		= $message; 
        $this->sender->strMobile		= (int)$reciever_phone;
        $sess = $this->sender->Submit();
        $sess = explode("|" , $sess );
        ...
        if ($sess[0] !== null) {
            ...
            echo '<br>';
            if ($sess[0] == "1701") {
                echo "successnmessage ID: ". $sess[1];
            } else {
                echo "send message failed";
            }
        } else {
            echo "Authentication failure: ". $sess[0];
        }
    }
